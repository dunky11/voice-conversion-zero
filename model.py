...
class DecoderWithTransformer(nn.Module):
    def __init__(self, in_channels, n_speakers, speaker_embedding_dim,
        ...
        self.transformer1 = torch.nn.TransformerEncoderLayer(
            ...
            nhead=8, 
            dim_feedforward=(in_channels + speaker_embedding_dim) * 4, 
            dropout=0.1, activation='relu')
        self.positional_embedding = PositionalEncoding(
            in_channels
        )
        self.mu_embedding = nn.Embedding(self.quantization_channels, mu_embedding_dim)
        self.rnn2 = nn.GRU(mu_embedding_dim + (in_channels + speaker_embedding_dim), rnn_channels, batch_first=True)
        self.fc1 = nn.Linear(rnn_channels, fc_channels)
        self.fc2 = nn.Linear(fc_channels, self.quantization_channels)
    ...
    def forward(self, x, z, speakers):
        ...
        z = torch.cat((z, speakers), dim=-1)
        z = z.permute(1, 0, 2)
        # transformer expects input of shape (seq, batch_size, embed)
